...
module.exports = {
        ...
        userModel.find({}, function (err, users) {
            if (err) {
                ...
                for (let user of users) {
                    usersList.push({ id: user._id, name: user.name, email: user.email, dob: user.dob, address: user.address, state: user.state, created_at: user.created_at });
                }
    ...
    updateById: function (req, res, next) {
        userModel.findByIdAndUpdate(req.params.userId, { name: req.body.name, dob: req.body.dob, email: req.body.email, dob: req.body.dob, address: req.body.address, state: req.body.state, created_at: req.body.created_at }, function (err, userInfo) {
            if (err)
                next(err);
            else {
